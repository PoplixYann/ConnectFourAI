...
{
    static Visual2D instance;
    public static Visual2D Instance
    {
        get
        {
            return instance;
    ...
    void MoveCurPiece()
    {
        if (curPieceGO != null)
        {
            curPieceGO.transform.position = Vector2.Lerp(curPieceGO.transform.position, GetTopColumnPos(curColumn), Time.deltaTime * curPieceSpeed);
            if (Vector2.Distance(curPieceGO.transform.position, GetTopColumnPos(curColumn)) <= 0.025f)
            {
    ...
    void AddPiece(int _line, int _column, Team _team)
    {
        if (IsUserPlayed(_team)) //Is user turn
        {
            StartCoroutine(AddPieceUserCoroutine(_line, _column));
        }
        else //Is AI turn
        {
            StartCoroutine(AddPieceAICoroutine(_team, _line, _column));
        }
    }
    ...
    IEnumerator AddPieceUserCoroutine(int _line, int _column)
    {
        ...
        //Let user piece fall
        curPieceGO.GetComponent<Rigidbody2D>().isKinematic = false;
    ...
    void SpawnNewPiece(Team _team, int _column)
    {
        ...
        curPieceGO.GetComponent<Rigidbody2D>().isKinematic = true;
        curPieceGO.transform.position = GetTopColumnPos(_column);
    ...
    Vector2 GetTopColumnPos(int column)
    {
        Vector2 pos = new Vector2(transform.position.x - (6 / 2) + column,
            transform.position.y + 7f);

        return pos;
    }

    Vector2 GetCellPos(int line, int column)
    {
        Vector2 pos = new Vector2(transform.position.x - (6 / 2) + column,
